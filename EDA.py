r"""
Streamer Game & Schedule Analytics Pipeline
============================================
Reads the monthly Steam / Twitch / game-catalog / GPU-price snapshots and:
  1. Reproduces the original EDA report charts (Q1-Q9 + distribution), fixed
     and made robust to missing/corrupt months.
  2. Builds per-game and per-timeslot recommendation tables and exports them
     as JSON for the companion website (site/index.html), which answers the
     two questions a streamer actually asks: "what should I play?" and
     "when should I go live?"

Fixes vs. the original EDA.py
------------------------------
- Crash-safety: monthly files can be empty/corrupt/missing (this dataset's
  own 2026-06 Steam file is 0 bytes and 2026-06 has no Twitch folder at all).
  `load_data` now skips unreadable files with a warning instead of blowing up
  the whole pipeline on `pd.concat`.
- Merge blow-up bug: `game_data` repeats every game once per month, so
  merging the fact table against it on `Game_Name` without de-duplicating
  first created a many-to-many join (rows were silently multiplied,
  quietly inflating the Random-Forest training set and skewing importances).
  Game info is now deduplicated to one row per game (latest month wins)
  before every merge.
- Redundant work: `q3_ml_influence` and `weight_analysis` each fit an
  identical RandomForestRegressor from scratch. The model is now fit once
  and shared.
- `os.chdir()` side effects: the original script changed the process's
  working directory to save each chart, which is fragile (an exception mid
  loop leaves you stranded in a deep sub-folder) and not thread/process
  safe. Every chart function now takes an explicit `out_dir`.
- Hour parsing used a regex (`str.extract(r'(\d{2})')`) for something that
  is just the first two characters of a fixed-format string; replaced with
  a plain slice, which is faster and equally correct.
- No caching: each monthly loop re-read and re-concatenated every CSV/parquet
  from scratch. Raw loading now happens once; monthly slicing reuses the
  in-memory frames.
- Parquet engine resilience: if `pyarrow`/`fastparquet` aren't installed,
  we fall back to a small pure-Python parquet reader
  (`parquet_fallback.py`) instead of crashing on import.
- New: exports `site/data.json` (game + schedule recommendation data) so the
  dashboard doesn't need a live backend — open `site/index.html` directly.
"""
import os
import glob
import json
